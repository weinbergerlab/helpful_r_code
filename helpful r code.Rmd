...
Pulling otu the summary statistics gets a bit trickier here. We have to use nested sapply statements to pull everything out by age group and then by model.
```{r}
mod.nobs<-sapply(mod.list2, function(x) sapply(x, '[[' , 'mod.nobs'))
mod.nobs
```
```{r}
mod.aic<-sapply(mod.list2, function(x) sapply(x, '[[' , 'aic.mod'))
mod.aic
```
