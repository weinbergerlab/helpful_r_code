...
We could combine these results into a list, and then use the functions from above
```{r}
mod.list<-list(mod1, mod2, mod3)
mod.coef<-lapply(mod.list, function(x) coef(x))
mod.coef 
```
...
```{r}
mod.aic<-sapply(mod.list, function(x) AIC(x))
mod.aic 

mod.nobs<-sapply(mod.list, function(x) nobs(x))
mod.nobs
```
